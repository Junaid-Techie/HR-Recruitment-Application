...
(function () {
    let CountryProfile = {
        populateDataTable: function (data) {
            ...
            var table = $('#data').DataTable({
                "data": data,
                "columns": [{ "data": "country_name" }, { "data": "country_code" }, { "data": "desc" },
                {
                    "data": "status",
                    "sWidth": "5%",
            ...
            $(document).ready(function () {
                $('#submit').click(function (e) {
                    ...
                    $.ajax({
                        url: "/hroffice/api/country/addrecord",
                        type: 'POST',
                        cache: false,
                        processData: false,
                        contentType: false,
            ...
            $(document).ready(function () {
                $('#updsubmit').click(function (e) {
                    ...
                    $.ajax({
                        url: "/hroffice/api/country/putrecord",
                        type: 'PUT',
                        cache: false,
                        processData: false,
                        contentType: false,
            ...
            $('#data tbody').on('click', 'tr', function () {
                if ($(this).hasClass('selected')) {
                    ...
                    console.log(table.rows('.selected').data()[0]['country_name']);
                    selected_value = table.rows('.selected').data()[0]['country_name'];
                    status = table.rows('.selected').data()[0]['status'];
                    txn = table.rows('.selected').data()[0]['country_name'];
                    ...
                    $('#updatebtn').click(function () {
                        ...
                        var txn = document.getElementById('txn')
                        txn.value = selected_value;
                        $('#target').attr('action', '/hroffice/editcountry');
            ...
            $('#update_status').click(function (row_number) {
                var rowData = table.rows('.selected').data()[0]['country_name'];
                ...
                url = "/hroffice/api/country/editstatus";

                $.ajax({
                    "url": url,
                    method: 'PUT',
                    data: { "country_name": rowData, "status": status, },
                    beforeSend: function () {
                        $('#loading').show();
                    },
                    complete: function () {
        ...
        loadData: function (successCallback) {
            url = '/hroffice/api/country/getrecords'
            $.ajax({
                type: 'GET',
                url: url,
                contentType: "application/json",
    ...
    $('#target').ready(function () {
        $.noConflict();
        CountryProfile.loadData(CountryProfile.populateDataTable);
    });
})();
