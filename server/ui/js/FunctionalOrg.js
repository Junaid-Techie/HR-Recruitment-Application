...
(function () {
    ...
    let FuncOrgProfile = {
        populateDataTable: function (data) {
            ...
            var table = $('#data').DataTable({
                "data": data,
                "columns": [{ "data": "functional_org" }, { "data": "org_email" }, { "data": "cc_email" },
                {
                    "data": "status",
                    "sWidth": "5%",
            ...
            $(document).ready(function () {
                $('#submit').click(function (e) {
                    ...
                    $.ajax({
                        url: "/hroffice/api/funcorg/addrecord",
                        type: 'POST',
                        cache: false,
                        processData: false,
                        contentType: false,
            ...
            $(document).ready(function () {
                $('#updsubmit').click(function (e) {
                    ...
                    var frmData = new FormData(frm)
                    $.ajax({
                        url: "/hroffice/api/funcorg/putrecord",
                        type: 'PUT',
                        cache: false,
                        processData: false,
                        contentType: false,
            ...
            $('#data tbody').on('click', 'tr', function () {
                if ($(this).hasClass('selected')) {
                    ...
                    console.log(table.rows('.selected').data()[0]['functional_org']);
                    selected_value = table.rows('.selected').data()[0]['functional_org'];
                    status = table.rows('.selected').data()[0]['status'];
                    txn = table.rows('.selected').data()[0]['functional_org'];
                    ...
                    $('#updatebtn').click(function () {
                        ...
                        var txn = document.getElementById('txn')
                        txn.value = selected_value;
                        $('#target').attr('action', '/hroffice/editorg');
            ...
            $('#update_status').click(function (row_number) {
                var rowData = table.rows('.selected').data()[0]['functional_org'];
                ...
                url = "/hroffice/api/funcorg/editstatus";
                //e.preventDefault();
                //var formData = new FormData($(this).parents('form')[0]);
                $.ajax({
                    "url": url,
                    method: 'PUT',
                    data: { "org_name": rowData, "status": status, },
                    success: function (data, textStatus, jQxhr) {
                        $('#message').html(data.message);
                        console.log(data);
                        if ($('#update_status').text() == "Mark Disable") {
        ...
        loadData: function (successCallback) {
            url = '/hroffice/api/funcorg/fetchrecord'
            $.ajax({
                type: 'GET',
                url: url,
                contentType: "application/json",
    ...
    $('#target').ready(function () {
        $.noConflict();
        FuncOrgProfile.loadData(FuncOrgProfile.populateDataTable);
    });
})();
