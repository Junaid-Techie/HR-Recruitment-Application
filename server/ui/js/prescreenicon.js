...
(function () {
    ...
    let PrescreenProfile = {
            ...
            var table = $('#data').DataTable({
...

                {
                    ...
                    "mRender": function (data, type, row, meta) {
                        ...
                        if (status == 'New') {
                            options = "<option value=" + status + " selected>" + status + "</option><option value='Accept'>Accepted</option><option value='Hold'>Hold</option><option value='Reject'>Rejected</option>"
                        } else if (status == 'Accept') {
                            options = "<option value=" + status + " selected>Accepted</option><option value='Hold'>Hold</option><option value='New'>New</option><option value='Reject'>Rejected</option>"
                        } else if (status == 'Reject') {
                            options = "<option value=" + status + " selected>Rejected</option><option value='Accept'>Accepted</option><option value='Hold'>Hold</option><option value='New'>New</option>"
                        } else {
                            options = "<option value=" + status + " selected>Hold</option><option value='Accept'>Accepted</option><option value='New'>New</option><option value='Reject'>Rejected</option>"
                        }
        ...
        loadData: function (successCallback) {
            url = '/hroffice/api/prescreen/getnotaccepted';
            $.ajax({
                type: 'GET',
                url: url,
